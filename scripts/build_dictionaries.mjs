...
async function buildJmdict(xmlPath) {
  const parser = new XMLParser({
    ...
    attributeNamePrefix: '@_',
    textNodeName: '#text',
    // Critical: JMdict uses DTD entities for POS/etc (e.g. &prt;).
    // If entities are processed, they expand to English descriptions ("particle"),
    // losing the stable POS tag codes needed for filtering.
  ...
  function normalizeEntityTag(t) {
    // JMdict POS/misc/field/dial often come through as entity refs like "&prt;".
    // Normalize to "prt" so runtime can filter by stable tag strings.
...
async function main() {
  ...
  for (const [k, v] of Object.entries(wordsMap)) {
    ...
    buckets[idx].push([k, v.reading, v.meaning, v.pos ?? []]); // [surface, reading, meanings, pos]
  }
  ...
  lines.push('export const WORD_BUCKETS: Record<string, readonly [string, string, string[], string[]][]> = {');
  for (let i = 0; i < bucketCount; i++) {
